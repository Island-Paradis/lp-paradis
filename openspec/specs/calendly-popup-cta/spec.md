# calendly-popup-cta Specification

## Purpose

O que acontece quando um CTA authorado no CMS aponta para o Calendly: o calendário abre num popup por cima da página, em vez de levar o visitante para fora dela.

Duas restrições moldam tudo aqui, e nenhuma é sobre o Calendly.

A primeira é **custo em repouso**. Um widget de terceiros montado em todas as rotas é a categoria de gasto que `idle-runtime-budget` existe para conter — trabalho ligado quando ninguém o está a ver. Daí o carregamento ser ao primeiro clique e não na montagem: uma página que ninguém clica não faz um único pedido a `assets.calendly.com`.

A segunda é que **o overlay não é nosso**. O `initPopupWidget` injecta DOM no `body`, não devolve handle e não aceita callback de fecho. Tudo o que esta capability exige em volta do popup — travar o scroll do fundo, saber quando ele fechou — tem de ser feito de fora, observando o DOM. É também por isso que ela declara explicitamente o que NÃO promete: foco, teclado e cursor dentro do overlay são do Calendly.

Esta capability é dona de "o que acontece ao clicar num destino Calendly". Que o destino chegue ao DOM é de [`cms-authored-cta-links`](../cms-authored-cta-links/spec.md).
